...
window.getReactRootInstances = window.getReactRootInstances || function getReactRootInstances() {
    var arr = window.getReactRoots().map(it => [...it.childNodes].map(el => el.reactInstance) || it._reactRootContainer._internalRoot.current.child.stateNode);
    var reactRootInstances = [];
    for(var el of arr) {
        if(el instanceof Array) {
            reactRootInstances = reactRootInstances.concat(el);
        } else {
            reactRootInstances.push(el);
        }
    }
    return reactRootInstances;
};
