...
window.toDecimals = function toDecimalsRaw(n, d) {
    n = (n && n.value || n);
    d = (d && d.value || d);
    if (!n || !d) {
        return "0";
    ...
    while(symbol) {
        try {
            ...
            n = n.split('.');
            n[1] && (n[1] = n[1].substring(0, decimals));
            n = n.join('.');
        }
    }
...
window.formatMoney = function formatMoney(value, decPlaces, thouSeparator, decSeparator) {
    ...
    var n = value,
        decPlaces = isNaN(decPlaces = Math.abs(decPlaces)) ? formatMoneyDecPlaces : decPlaces,
